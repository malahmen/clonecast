...
**4.15 The master is whichever window has focus: the engine resolves the origin once per broadcast and every deliverer skips it, gated on the origin being a ticked target.** (2026-09-11) Implements 7.10 and research §C1. `Deliverer.Deliver` takes an `origin WindowID`; the engine reads it once per broadcast key (`wm.Active`, ~1ms per 7.6) and hands it down instead of each backend re-querying, and every backend must skip it because passthrough already delivered the key there. Before this, only the dance skipped the origin, so a ticked *and* focused window received every broadcast key twice through the agent/xsend backends (research O1) — which is why ticking all clients was useless. Now the intended setup is "tick every client"; the master is simply the one you are looking at, and switching windows switches the master with no re-ticking (O2/O3). `Config.GateOrigin` (default on, `--gate-origin=false`, `g` in the TUI) additionally requires the focused window to be one of the ticked targets, so typing in the terminal that runs clonecast with broadcasting on drives nothing; the gate reports once per focused window rather than per keystroke. If `wm.Active` fails, the gate decides what happens: with it on, the event is reported and skipped (broadcasting blind would double-deliver to the master and silently ignore the user's "only from a target" rule on no evidence); with it off, delivery falls back to every ticked target with an empty origin, i.e. exactly the pre-origin behaviour. The dance re-queries for itself when handed an empty origin, because unlike the focus-free backends it must restore focus at the end. *Alternatives:* each deliverer calling `wm.Active` itself (rejected: N lookups per key, and no single place to gate); an agent-side foreground check as the only mechanism (rejected as sole mechanism per research §C1 — it lags the X focus change by a message-loop turn; it remains a worthwhile safety net on top).
