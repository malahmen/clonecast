...
**4.16 The prefix owns the agent's lifetime: autostart from `RunServices`, installed into `drive_c`.** (2026-09-11) `clonecast agent install --prefix <path>` copies `clonecast-agent.exe` to `<prefix>/drive_c/clonecast/` and registers it under `HKLM\Software\Microsoft\Windows\CurrentVersion\RunServices`. Verified against Wine source (research.md §C2/§G): every `wine <exe>` that starts a fresh wineserver implicitly runs `wineboot --init` (ntdll `run_wineboot()`), and `--init` processes `RunServicesOnce`, `RunServices` and `RunOnce` — but **not** `Run` and not the Startup folder, which need a bare `wineboot` that Bottles, Lutris, Proton and umu never run per launch. So this one key fires on every prefix boot under every launcher, with no launch-time integration: dark-portal's role in the agent's lifecycle drops to nothing (7.11). The agent then dies with the wineserver, so the host never signals it — which is what killed the game in 4.14. The binary must live under `drive_c`, not `~/.local/bin`: under Bottles' Flatpak `Z:` only exposes the sandbox's bind mounts, not `$HOME`.

Two install routes, picked automatically and reported: **(a)** edit `<prefix>/system.reg` directly (Wine's registry is a plain-text format, so no wine binary is needed) — only when no process holds that `WINEPREFIX`, because a live wineserver holds the registry in memory and rewrites the file on shutdown, discarding the edit; **(b)** shell out to a user-supplied wine command (`--wine "<cmd>"`, e.g. a `flatpak run --command=…/wine … com.usebottles.bottles` invocation or `umu-run`) to run `reg add`, for a prefix that is already up. Running prefixes are discovered by scanning `/proc/*/environ` for `WINEPREFIX=` (Linux-only, build-tagged), which also lets the TUI offer prefixes instead of asking for a path.

Consequences inside the agent, all in place: the window wait is **unbounded** with a slow poll (it now starts before the game, so the old 60 s timeout was an exit-before-the-game bug); it takes a **named mutex** (`Local\clonecast-agent-<port>`) because `wineboot` processes the run keys in both registry views on a 64-bit prefix and starts a single entry twice; and it logs to a file beside itself, since an autostarted process has no console. *Alternatives:* launcher-specific hooks (a dark-portal step, Steam launch options) rejected as coupling — clonecast must work with any launcher; the `Run` key rejected because `--init` skips it; a binary on `Z:` rejected because Bottles' Flatpak cannot see `$HOME`.

---

**4.15 The master is whichever window has focus: the engine resolves the origin once per broadcast and every deliverer skips it, gated on the origin being a ticked target.** (2026-09-11) Implements 7.10 and research §C1. `Deliverer.Deliver` takes a `Delivery` request struct carrying the event, the targets, the origin and the notice sink; the engine reads the origin once per broadcast key (`wm.Active`, ~1ms per 7.6) and hands it down instead of each backend re-querying, and every backend must skip it because passthrough already delivered the key there. A struct rather than a fifth positional parameter because `Targets []WindowID` and `Origin WindowID` read identically at a call site, and this interface has now grown twice (Google Go style guide, "function argument lists"; context stays first and outside the struct). Before this, only the dance skipped the origin, so a ticked *and* focused window received every broadcast key twice through the agent/xsend backends (research O1) — which is why ticking all clients was useless. Now the intended setup is "tick every client"; the master is simply the one you are looking at, and switching windows switches the master with no re-ticking (O2/O3). `Config.GateOrigin` (default on, `--gate-origin=false`, `g` in the TUI) additionally requires the focused window to be one of the ticked targets, so typing in the terminal that runs clonecast with broadcasting on drives nothing; the gate reports once per focused window rather than per keystroke. If `wm.Active` fails, the broadcast is skipped and reported — **fail closed, whatever the gate says**. An earlier revision of this change fell back to "deliver to every ticked target with an empty origin" when the gate was off, on the reasoning that the user had already said focus decides nothing; that was wrong. With no origin nothing is skipped, so the client being played receives every key twice — precisely the bug 4.15 exists to remove — and a doubled keystroke in a game is not undoable, while a suppressed one costs a single keypress. This is fail-safe defaults (Saltzer & Schroeder 1975; OWASP "fail securely") applied to a tool that types into other people's applications. Because failing closed silently is how a tool gets reported as broken (see 4.9's "broadcasts never arrive despite the log saying they did"), the suppression is always reported, throttled to once per run of failures. Deliverers are therefore never handed an unresolved origin, so none of them needs a fallback path. *Alternatives:* each deliverer calling `wm.Active` itself (rejected: N lookups per key, and no single place to gate); fail-open on an unresolved origin (rejected above); an agent-side foreground check as the only mechanism (rejected as sole mechanism per research §C1 — it lags the X focus change by a message-loop turn; it remains a worthwhile safety net on top).

**4.18 The agent backend is the default `--deliver`; the dance is a documented last resort.** (2026-09-11, numbered past 4.16/4.17, which research §D reserves for the agent autostart and self-registration work) Doc and code disagreed on the mainline (research I11: `--deliver` defaulted to `dance` while 4.12/7.9 declared the agent the reliable default). Flipped: bare `clonecast` selects `agent` and fails fast with an actionable message naming `--agent` and the `--deliver dance` fallback when no endpoints are configured, rather than starting up and broadcasting nowhere. `--backend mock` still defaults to `dance`, since a mock desktop has no Wine prefixes and therefore no agents — that is what keeps `make run-mock` self-contained. Backends are also switchable at runtime from the TUI's Settings pane (built once, cached, swapped under a lock), so trying the dance after the agent no longer means restarting.
